        ...
        async with conn.cursor() as cur:
            await cur.execute("""
                SELECT sn.*,
                  ...
                  (SELECT timestamp   FROM sensor_data WHERE node_id=sn.node_id ORDER BY timestamp DESC LIMIT 1) AS last_seen,
                  (SELECT rssi        FROM sensor_data WHERE node_id=sn.node_id ORDER BY timestamp DESC LIMIT 1) AS last_rssi
                FROM sensor_nodes sn WHERE sn.is_active=TRUE ORDER BY sn.node_id
            """)
    ...
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            await cur.execute("""
                ...
                WHERE sd.timestamp=(SELECT MAX(timestamp) FROM sensor_data WHERE node_id=sd.node_id)
                ORDER BY sd.node_id
            """)
            rows = await cur.fetchall()
            cols = [d[0] for d in cur.description]
    ...
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                ...
                f"FROM sensor_data WHERE node_id=%s {time_filter} ORDER BY timestamp ASC",
                (node_id,)
            )
            rows = await cur.fetchall()
...
async def add_sensor_data(body: SensorDataIn, _=Depends(verify_gateway_key)):
    """Endpoint untuk LoRa gateway — dilindungi API key"""
...
async def get_node_detail(node_id: str, user=Depends(get_current_user)):
    ...
    node["stats_24h"] = {
        ...
        "min_temp": stats[2], "total": stats[3], "danger_count": stats[4],
    }
...
async def create_node(body: SensorNodeCreate, user=Depends(require_pic_or_admin)):
    ...
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            try:
                await cur.execute(
                    "INSERT INTO sensor_nodes (node_id, node_name, location, is_active) VALUES (%s,%s,%s,%s)",
                    (body.node_id, body.node_name, body.location, body.is_active)
                )
                new_id = cur.lastrowid
            except Exception:
                raise HTTPException(status_code=409, detail=f"node_id {body.node_id} sudah terdaftar")
